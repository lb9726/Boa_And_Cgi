...
//        connect(this, &WifiCommunication::sigCloseSocket, mMyTcpServer, &MyTcpServer::slotCloseTcpSocket);
    }
...
{
    qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<buffer;
...
void WifiCommunication::unZipStatus(QString str)
{
    QJsonObject typeObj;

    typeObj.insert("type", "unZip");
    typeObj.insert("info", str);

    QByteArray sendFileByte = convertJsonToByteArray(typeObj);
    qDebug()<<__PRETTY_FUNCTION__<<"sendFileByte = "<<sendFileByte<<"str = "<<str;
    sendFileByte = generateBuffer(sendFileByte);
    writeMsgToClient(sendFileByte, sendFileByte.length());
}
...
{
    int flag = system("ifconfig wlan0 up 192.168.1.10");
    return flag;
}

int WifiCommunication::stopWifi()
{
    int flag = system("/etc/init.d/S40network stop");
    return flag;
}
...
void WifiCommunication::onDeviceDisconnected()
{
    if (mHeartBeatTimer.isActive())
    {
        qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<"mHeartBeatTimer is Active will stop";
        mHeartBeatTimer.stop();
    }
//    emit sigCloseSocket();
